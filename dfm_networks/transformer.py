...
from dfm_networks.utils.swinblock import SwinTransformerBlock
...
class LGUnet_all(nn.Module):
    ...
    def multi_forward(self, data, steps):
        predictions = []
        for i in range(steps):

            pred = self.forward(data)
            # pred = checkpoint(self.forward, data)
            predictions.append(pred)

            data, _ = pred.chunk(2, dim = 1)
        
        return torch.cat(predictions, dim=0)


class ExtraLogVar(nn.Module):
    def __init__(self, in_channels=69, out_channels=69):
        super().__init__()
        ...
        self.logvar_head = nn.Conv2d(64, out_channels, kernel_size=1)

    def forward(self, x):
        feat = self.encoder(x)       # [B, 256, 32, 64]
        feat = self.decoder(feat)    # [B, 64, 32, 64]

        logvar = self.logvar_head(feat)      # [B, 69, 32, 64]
